...
The Unicode Basic Multilingual Plane, or BMP, is the original 65536 character plane, the only character plane available in Unicode 1.0
...
Vladislav Vaintroub commented to me:
...
Extending Unicode beyond the BMP posed a bit of a problem, because Windows NT had started to use Unicode as a character set, and ucs-2 as the standard encoding in Windows NT.
...
The high ten bits are added to 0xd800 (0xd800-0xdbff), the low then bits are added to 0xdc00 (0xdc00-0xdfff).
This was called utf-16 encoding, and became the character set of Windows starting with Windows 2000.

utf-16 also, for now, limits the number of characters that can be represented in Unicode encodings to a number far below 31 bit.

## Plan 9 and utf-8

In the Unix world, the experimental Plan 9 operating systems from A&T explored variable width encodings of Unicode:
...
Because the utf-16 hack had restricted the number of possible Unicode characters effectively from 31 bit to 21 bit, utf-8 encodings can only be up to 4 bytes long.
...
In a utf-8 multibyte sequence, the "number of 1-bits before the first 0-bit" in the first byte of a multibyte character determine the length of the encoding:
...
This makes utf-8 restartable: If you end up at a byte of the form 10xxxxxx, you can backtrack a few bytes and check for the clearly recognizable start pattern of a multibyte encoding, or error out with an invalid encoding.
...
That was changed [in a single commit](https://github.com/mysql/mysql-server/commit/43a506c0ced0e6ea101d3ab8b4b423ce3fa327d0) in September 2002.
...
## Character sets have sort and comparison rules, called collations
...
Originally, there was just one type of comparison defined, and it was used for sorting and equality tests.  
So if "Köhntopp" is supposed to sort like "Kohntopp", it would also return `true` for the comparison `WHERE "Köhntopp" = "Kohntopp"`.
That is clearly wrong and annoying.
[lefred's Blog article](https://lefred.be/content/mysql-character-sets-unicode-and-uca-compliant-collations/) and [Manyi Lu's article](https://dev.mysql.com/blog-archive/sushi-beer-an-introduction-of-utf8-support-in-mysql-8-0/) about the [Sushi-Beer issue](https://bugs.mysql.com/bug.php?id=76553) explain this in depth.
...
That is, because the moment you apply the upgrade, you are forced to drop and recreate the indexes.
Depending on the amount of data that requires an extraordinary amount of time and makes updates complicated.

# `utf8mb4`, because `utf8` was taken.

As a policy, MySQL these days never updates existing character sets and collations, not even for bugfixes.
...
With MySQL and the immutable character set and collation rule, you can create new columns using the new character set and collations, even in existing tables.
That is possible, because in MySQL a charset and collation is a column attribute.
...
Nothing would change, you are just running new code that hopefully sorts and orders as before.

Then you would check all columns, and note which ones need changing.
