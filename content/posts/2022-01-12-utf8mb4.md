...
In InnoDB, the row pointer used by secondary indexes is actually the primary, `id`, so we do het the index `c` as tuples `(c, id)`.
...
Our collation is given as `utf8mb4_0900_ai_ci`, and that are the Unicode Collation Algorithm (UCA) 9.0 sorting rules for 4-byte UTF*, with accent insensitivity and case insensitivity.
...
`ai` means accent insentitive, so `WHERE c = "Kohntopp"` matches "Köhntopp". With `as` they would not.
...
This is another important fact we need to remember.
...
That means, while `utf8` is a strict subset of `utf8mb4` in terms of character repertoire and encoding, it sorts differently.
...
## So what would happen if `utf8` suddently means `utf8mb4`?
...
Tables, databases and servers just provides defaults that are inherited at the time of object creation.

So if you wanted to upgrade your database, you'd installa new version.
...
For Unicode that means, 4-byte Unicode is `utf8mb4` in MySQL.
...
Thaty is because they keep the state for your program, so that your program can be stateless and easy to manage.
...
Across all results we perform a merge sort.
...
Older versions of MySQL has different storage engines: `MYISAM`, `INNODB`, `MEMORY` and others.
...
Of course, with `utf8`, a `varchar(255) charset utf8` is promoted to a `char(255) charset utf8`, which instead allocates 765 bytes of memory, out of which the first five bytes contain the string `hello` and the rest is padding.
