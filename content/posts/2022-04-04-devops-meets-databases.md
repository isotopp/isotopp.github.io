...
Asking about what he wanted to know specifically he said, he's interested in the reasons why DevOps and Agile takeup in DBA circles is so slow and low.
That forced me to brain dump stuff that has been active in the back of my mind for quite some time now, but that I never collected proper.
...
The talk basically explains what happened in the 10 years after the Dotcom Boom, and how the adoption of "scale-out" instead of "scale-up" created the need to Sysadmins to pick up Developer tooling and some developer methods in order to deal with machinery in numbers. 
...
But operations people still are judged by resiliency and handling failures, so handling worst cases.
The gap in mindset between Developers and Operations is hard to bridge, but it can be done with a little tricker.
In any case, the traditional Sysadmin job died out.

Some people ask about the distinction between "DevOps vs. SRE", and there is very little -- for some, "SRE" is the Google employer branding for the "DevOps" profile.
Others make vague distinction that riff on the fact that "SRE" are people that care about worst cases, resiliency and fighting operational toil, and that also care about observability.
In my experience that is too subtle a distinction to hold up in a hiring interview.

# DevOps and Databases

With regards to databases, this development did not happen at the same speed.
...
In fact, dealing with state, with transactions and with persistence is there only purpose in life.
These things make proper database automation hard, and most existing DevOps tooling unsuitable for the task.
...
So reconnecting to the database transparently and hiding the disconnect/reconnnect from the application is almost impossible, and developers are hating the database for this (or ignore the problem, producing incorrect code).
...
That is a legal wish to have, if you can afford the cost, because it makes development easier and a lot more boring. 
...
## Automating databases is hard, because of inter-node dependencies

Scaling out databases and automating their operation is a hard task. 

Not only are connections stateful, things like SQL databases, Zookeepers, etc, Consul (consensus systems, in general), and things that have consensus systems inside are clusters. 
...
The point being that making changes to such a cluster creates an order to updates as they are applied, and checkpoints that need to be passed for the change to proceed.
...
And updates to such clusters can only be made while maintaining quorum, so after application of each change the update orchestrator needs to check if the node it just worked on came back properly from the upgrade before proceeding with the next node.
All of the above tools have no concept of such constraints, they are simply unfit to deal with databases that are not a SPOF.

Ansible can dig itself out of that hole by the sequential and slow nature of its processing, but is also unaware of the actual topology constraints.
...
Also, working with databases is always slow, [because data has weight]({{< ref "/content/posts/2022-02-16-databases-how-large-is-too-large.md" >}}). At 400 MB/s, copying 1 TB takes 45 minutes, and replication catchup and cache warmup take another 15 minutes, so we can calculate and communicate "1 hour per 1 TB of data size".
...
And you still did not have the plan, or the number of actual pages read for this single query.
...
Oracle for example never ran their own products at scale, at least MySQL as a company, and later as adivision in Oracle never did.
...
On the DBA side: most DBA departments positioned themselves as "guardians of the data" and chokepoints, like old sysadmins did before DevOps.
...
Mostly, I believve because there was never sufficient pressure.
...
So there is a big divide between Developers as users of a database, DBAs, and vendors/bloggers/commmunity advocates.
