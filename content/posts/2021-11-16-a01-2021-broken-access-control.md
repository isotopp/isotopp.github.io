...
- **Identität:** Ich bin ich.
  ...
  Eine Anwendung muß das an meiner Statt tun.
...
- **Authentisierung:** Ich kann das auch beweisen.
  ...
  Ich bestätige meine Identität meist mit einem Paßwort und oft auch mit einem 2. Faktor (etwa einer vom einem Google Authenticator abgetippten wechselnden Geheimnummer).

Die behauptete bestätigte Identität muß für einige Zwecke auch noch **verankert** werden, etwa wenn diese Identität an andere Rechte oder Verhältnisse in der realen Welt gebunden werden soll.
Wenn ich zum Beispiel in einer Schul-App eine Schulzugehörigkeit und Klassenzugehörigkeit haben soll, dann ist es unter Umständen gut, wenn ich mir nich einfach einen Account machen kann.
...
- Was ich dann in der Anwendung tue muß unter Umständen beweiskräftig mitgeschnitten werden.
  Etwa dann, wenn die Daten, auf die ich zugreife besonders schützenswert sind, oder ihre Änderungshistorie dokumentiert werden muß.
...
- Wenn ich Dienstleistungen in Anspruch nehme, die Geld kosten, dann muß eine besondere Form von Audit-Log geführt werden.
...
Man kann zu jedem dieser Themen viel schreiben, aber uns soll hier in erster Linue *Autorisierung* beschäftigen, eben weil es Thema #1 bei der aktuellen OWASP Liste ist.
...
Damit meine ich nur nur die HTMLWebseiten (von denen ja einige auch interativ sind und Parameter haben können), sondern auch die API, die den Server für eine App auf einem Telefon oder in einem Browser repräsentiert.
...
Nämlich immer dann, wenn die interne Struktur innerhalb der Anwendung nicht weiter segmentiert werden muß.
...
1. Ich muß wissen, welche Außenfläche meine Anwendung hat, also welche URLs mit welchen Parametern zugelassen werden.
...
3. Ich muß meine Anwender identifizieren (ihnen also eindeutige Identifier zuweisen) und authentisieren (sie müssen die behauptete Identität also beweisen).
4. Die Liste dieser Anwender ist dann die Liste meiner Subjekte.
5. Ich muß nun ACL, RBAC oder ABAC Regeln die definieren, "Wer was mit wem" machen kann und dies im Access Control Framework meiner Anwendung ausdrücken.
...
Ich erschlage also nicht nur alle Compliance-, viele Datenschutz und alle Security-Pflichten, sondenr ich habe den kompletten Data Lifecycle meiner Anwendung mit einem Zustandsübergangsdiagramm dokumentiert,
