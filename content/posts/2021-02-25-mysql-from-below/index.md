...
XFS also is capable of having multiple concurrent writers on a file when the file is open with O_DIRECT. Unlike all other Linux File Systems, [it does not lock the file’s in-memory inode globally]({{< relref "2018-11-29-but-is-it-atomic.md" >}}) to guarantee atomic writes in this case. Such a feature is very dear to database people.
...
The chosen example is a [database as a queue]({{< relref "2021-01-28-database-as-a-queue.md" >}}), that logs changes from one data set for processing and creation of a materialized view in another data set. It is an example of [CQRS](https://martinfowler.com/bliki/CQRS.html) in Fowlerspeak.
